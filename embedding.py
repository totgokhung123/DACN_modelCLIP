    ...
    for root, _, files in os.walk(folder_path):  
        for file in tqdm(files):  
            if file.lower().endswith(('.jpg', '.jpeg', '.png')):  
                ...
                # Cập nhật embedding vào document tương ứng hoặc thêm mới nếu không có  
                result = frames_collection.update_one(  
                    {"filepath": image_path},  # Điều kiện tìm frame  
                    {"$set": {"embedding": embedding}},  # Cập nhật embedding  
                    upsert=True  # Thêm mới nếu không tìm thấy document  
                )  
